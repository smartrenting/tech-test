...
import { getAveragePizzaByOrder } from "./helper";

const AveragePizzaByOrder = () => {
  const { ordersState } = useAppContext();
  const { orders } = ordersState;
  const [average, setAverage] = useState(0);

  useEffect(() => {
    setAverage(getAveragePizzaByOrder(orders));
  }, [orders]);
  return (
    <div className="average">
      <h2>Avarega pizza per order</h2>
      <p>
        {`People order ${average}`}
        <svg
          aria-hidden="true"
          focusable="false"
          data-prefix="fas"
...
export default AveragePizzaByOrder;
